    ...
    <>
      ...
      <div className="hidden lg:flex lg:w-64 lg:flex-col lg:fixed lg:inset-y-0 lg:z-50">
        <Card className="flex flex-col h-full gradient-card border-r border-border/50 rounded-none">
          {/* Logo */}
          <div className="flex items-center justify-center h-16 px-6 border-b border-border/50 flex-shrink-0">
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <Button
                  ...
                  className={`w-full justify-start h-12 transition-all duration-200 flex-shrink-0 ${
      ...
      <div className="lg:hidden">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <Card className="absolute top-16 left-0 right-0 z-50 gradient-card border-b border-border/50 rounded-none animate-slide-up max-h-[calc(100vh-4rem)]">
            <nav className="px-4 py-4 space-y-2 overflow-y-auto max-h-[calc(100vh-8rem)] scrollbar-hide">
              {navigationItems.map((item) => {
                const Icon = item.icon
                const isActive = currentPage === item.id
